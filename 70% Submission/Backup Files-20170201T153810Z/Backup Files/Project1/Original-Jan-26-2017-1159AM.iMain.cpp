...
char mainmenu[] = "background\\main menu1.bmp.bmp";
...
struct imagelocation{
	...
	char location[100];
	int positionXX = rand() % (HEIGHT - ballheight+200);
	...
	void setposition(){
		...
		if (positionYY <= 0 ){
			positionXX = rand() % (HEIGHT - ballheight+200);
			positionYY = HEIGHT;
		}
		else
			positionYY -= ballmove;
...
struct boss{
	...
	void setlocationnn1(){
		xstrcpy(locationnn1, "background\\dragon1.bmp.bmp");
	...
	void fireballsetposition(){
		...
		if (fireballX <= 0){
			...
			fireballX = positionXXXX1 - 120;
			fireballY = positionYYYY1 - 100;

		}
		if (fireballY <= 0){

			fireballX = positionXXXX1 - 140;
			fireballY = positionYYYY1 - 100;
...
void iDraw()
{
	...
	if (gamestart == true){
		...
		if (levelup >= 51){
			...
			if (bossExist != true){
				iShowBMP(boss1.positionXXXX1, boss1.positionYYYY1, boss1.locationnn1);
			}
...
{
	if (mx >= 330 && mx <= 524 && my >= 277 && my <= 329){
		gamestart = true;
	}
